...
class AnalyticsError(Exception):
    """Erro específico do pipeline de analytics."""
...
def load_sales_dataframe(data_file: str) -> pd.DataFrame:
    ...
    if df.empty:
        raise AnalyticsError("Dataset de vendas está vazio")
...
def build_feature_frame(df: pd.DataFrame, product: Optional[str] = None) -> pd.DataFrame:
    if product:
        ...
        if df.empty:
            raise AnalyticsError(f"Produto '{product}' não possui dados suficientes")
...
def generate_analytics_snapshot(data_file: str, product: Optional[str] = None) -> Dict[str, Dict[str, float]]:
    try:
        ...
        logger.warning("Pipeline analytics retornou erro de negócio: %s", error)
        raise
    except Exception as exc:  # pylint: disable=broad-except
        logger.exception("Falha ao executar pipeline de analytics")
        raise AnalyticsError("Não foi possível processar o pipeline estatístico") from exc

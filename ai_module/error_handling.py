...
# Importar sistema de monitoramento se disponível
try:
...
class BaseAIException(Exception):
    """Classe base para todas as exceções do sistema AI."""
...
class AIAPIError(BaseAIException):
    """Exceção para erros de APIs de IA."""
...
class ModelLoadError(BaseAIException):
    """Exceção para erros ao carregar modelos."""
...
class ErrorHandler:
    ...
    def handle_error(self, 
        ...
        # Converte para nossa exceção padronizada se necessário
    ...
    def _convert_to_ai_exception(self, 
                                ...
                                context: Dict[str, Any] = None,
                                severity: ErrorSeverity = None) -> BaseAIException:
        """Converte exceção genérica para nossa exceção padronizada."""
        ...
        if 'connection' in error_message.lower() or 'network' in error_message.lower():
            ...
            return BaseAIException(
                f"Erro não categorizado ({error_type}): {error_message}",
                original_exception=error,
                context=context,
                severity=severity or ErrorSeverity.MEDIUM
            )
...
def retry_with_fallback(
    ...
    """
    Decorator para retry automático com fallback.
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(1, retry_config.max_attempts + 1):
                try:
                    ...
                    if attempt > 1:
                        logger.info(f"Função {func.__name__} executada com sucesso na tentativa {attempt}")
                    ...
                    if attempt < retry_config.max_attempts:
                        ...
                        logger.error(f"Função {func.__name__} falhou após {retry_config.max_attempts} tentativas")
            ...
            if fallback_function:
                try:
                    ...
                    logger.error(f"Fallback também falhou para {func.__name__}: {fallback_error}")
                    # Levanta erro original, não o do fallback
...
def error_boundary(
    ...
    """
    Decorator que cria boundary de erro para funções.
    
    Args:
        fallback_value: Valor retornado em caso de erro
        reraise: Se deve relançar a exceção após log
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                # Decide se relança ou retorna fallback
                if reraise:
                    raise ai_error
                else:
...
# Configurações de retry predefinidas
...
def format_error_for_user(error: BaseAIException) -> str:
    """Formata erro para exibição ao usuário final."""
    user_messages = {
        ...
        ErrorCategory.AI_API: "Serviço de IA temporariamente indisponível. Tente novamente.",
        ErrorCategory.VALIDATION: "Dados fornecidos são inválidos. Verifique e tente novamente.",
        ErrorCategory.FILE_SYSTEM: "Problema ao acessar arquivos do sistema.",
        ErrorCategory.CONFIGURATION: "Problema de configuração do sistema."
    }

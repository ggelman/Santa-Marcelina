...
# -*- coding: utf-8 -*-
"""
Módulo de cache Redis para o sistema de IA.
Implementa cache inteligente para modelos Prophet e predições.
"""
    ...
    def _connect(self):
        ...
        try:
            ...
            self.redis_client = redis.Redis(
                ...
                retry_on_timeout=False    # Não tentar novamente se falhar
            )
            ...
            logger.warning(f"⚠️ Redis não disponível: {e}. Funcionando sem cache.")
            self.enabled = False
            self.redis_client = None
...
class ModelCache:
    """
    Cache específico para modelos Prophet.
    """
    ...
    def invalidate_model(product_name: str):
        """Invalida cache do modelo e predições relacionadas."""
        # Remove modelo
        model_key = get_cache()._generate_key("model", product_name)
        get_cache().delete(model_key)
        
        # Remove predições relacionadas
    ...
    @staticmethod
    def invalidate_all():
        """Invalida todo o cache do módulo AI."""
        pattern = "ai_module:*"
...
def cached_model():
    """
    Decorator para cache automático de carregamento de modelos.
    """
    def decorator(func):
        @wraps(func)
...
def cached_prediction():
    ...
    def decorator(func):
        ...
        def wrapper(product_name: str, days_ahead: int = 1, *args, **kwargs):
            ...
            if cached_result is not None:
                logger.debug(f"Cache HIT para predição: {product_name} ({days_ahead} dias)")
                return cached_result
            
            # Se não encontrou, executa função original
            logger.debug(f"Cache MISS para predição: {product_name} ({days_ahead} dias)")
...
def warm_up_cache():
    """
    Pré-aquece o cache carregando modelos principais.
    """
    if not get_cache().enabled:
        logger.info("Cache não disponível, pulando warm-up")
        return
        ...
        for product in products:
            try:
                ...
                if model is not None:
                    warmed_up += 1
                    logger.debug(f"Modelo pré-carregado: {product}")
            except Exception as e:
                logger.warning(f"Erro ao pré-carregar {product}: {e}")
        
        logger.info(f"🔥 Warm-up concluído: {warmed_up}/{len(products)} modelos carregados")
        
    except Exception as e:
        logger.error(f"Erro durante warm-up: {e}")

# Função para health check do cache
...
# Instância global do cache (lazy loading)
cache = None

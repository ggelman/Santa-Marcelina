...
def recommend_seasonalities(df, date_col='ds'):
    ...
    if freq == 'hourly':
        ...
        rec['daily'] = False  # geralmente Prophet daily_seasonality para dados diários é ruído
        rec['weekly'] = True
    elif freq == 'weekly':
        rec['weekly'] = True
        rec['yearly'] = True
...
def detect_seasonality_patterns(df):
    """
    Detecta padrões de sazonalidade nos dados.
    
    Args:
        df: DataFrame com as colunas 'ds' e 'y'
    
    Returns:
        Dict com informações sobre padrões de sazonalidade detectados
    """
...
def get_local_holidays(start_date, end_date, country='BR'):
    """
    Obtém feriados locais para o período especificado.
    
    Args:
        start_date: Data inicial
        end_date: Data final
        country: Código do país (default: 'BR' para Brasil)
...
def configure_seasonality(model, patterns, threshold=0.1):
    """
    Configura a sazonalidade do modelo Prophet com base nos padrões detectados.
    
    Args:
        model: Modelo Prophet
        patterns: Dict com padrões de sazonalidade detectados
        threshold: Limiar para considerar um padrão significativo
    
    Returns:
        Modelo Prophet configurado
    """
    # Configura sazonalidade diária
...
def analyze_and_save_patterns(df, product_name, output_dir):
    """
    Analisa padrões de sazonalidade e salva os resultados.
    ...
    # Formata os resultados de forma mais legível
    readable_patterns = {
        'daily': {
        ...
        'weekly': {
            ...
            'peak_day': ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'][patterns['weekly']['peak_day']],
            'low_day': ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'][patterns['weekly']['low_day']]
        },
        'monthly': {

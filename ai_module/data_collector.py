...
if __name__ == '__main__':
    ...
    db_database = os.getenv('DB_DATABASE', 'padaria_santa_marcelina')
    db_user = os.getenv('DB_USER', 'root')
    db_password = os.getenv('DB_PASSWORD')

    if not db_password:
        print("ERRO CRÍTICO: A variável de ambiente DB_PASSWORD não está definida.")
        print("Crie um arquivo .env no diretório 'ai_module' e adicione a linha: DB_PASSWORD='sua_senha'")
        exit(1)
    ...
    if not sales_df.empty:
        ...
        if not processed_df.empty:
            print("Dados pré-processados para o Prophet:")
            ...
            print("AVISO: Os dados de vendas foram obtidos, mas resultaram em um conjunto vazio após o processamento.")
    else:
        print("ERRO CRÍTICO: Não foi possível obter dados de vendas do banco de dados.")
        print("Verifique a conexão, as credenciais e se a query no script retorna algum dado.")
        exit(1)

...
def get_business_context():
    """Obtém contexto atual do negócio através das APIs."""
    try:
        context = {}
...
def generate_smart_response(user_message, context):
    """Gera resposta inteligente usando OpenAI com contexto do negócio."""
    try:
        # Preparar contexto para o prompt
        context_text = f"""
        Contexto atual da Padaria Santa Marcelina:
        ...
        - Receita do mês: R$ {context.get('financeiro', {}).get('receita', 0):.2f}
        - Despesas do mês: R$ {context.get('financeiro', {}).get('despesa', 0):.2f}
        - Lucro do mês: R$ {context.get('financeiro', {}).get('lucro', 0):.2f}
        """
        
        system_prompt = f"""
        Você é um assistente inteligente especializado em gestão de padarias. 
        Você tem acesso aos dados em tempo real da Padaria Santa Marcelina e pode fornecer insights valiosos.
        
        {context_text}
        
        Responda de forma útil, prática e específica. Use os dados fornecidos quando relevante.
        Seja conciso mas informativo. Foque em ações práticas que o usuário pode tomar.
        """
...
def generate_fallback_response(user_message, context):
    """Gera resposta baseada em regras quando a IA não está disponível."""
    message = user_message.lower()
    
    if "vendas" in message or "venda" in message:
        total_vendas = context.get('total_vendas', 0)
        return f"Você tem {total_vendas} vendas registradas no sistema. Para análises detalhadas, acesse o relatório de vendas no menu principal."
    
    if "estoque" in message or "produto" in message:
        produtos_baixo = len(context.get('produtos_baixo_estoque', []))
        if produtos_baixo > 0:
            return f"⚠️ Atenção! Você tem {produtos_baixo} produtos com estoque baixo que precisam de reposição. Acesse a Gestão de Estoque para mais detalhes."
        else:
            return "✅ Seu estoque está em bom estado! Todos os produtos estão acima do nível mínimo."
    ...
    if "financeiro" in message or "dinheiro" in message or "lucro" in message:
        ...
        return f"💰 Situação financeira do mês: Receita de R$ {receita:.2f} e lucro de R$ {lucro:.2f}. Acesse o Dashboard Financeiro para análise completa."
    
    if "previsão" in message or "demanda" in message:
        return "🤖 Para previsões de demanda baseadas em IA, acesse o módulo 'Previsão IA' no menu. Lá você encontra previsões para todos os produtos."
    
    if "cliente" in message or "fidelidade" in message:
        return "👥 Para gerenciar clientes e o programa de fidelidade, acesse a seção 'Clientes' no menu principal."
    
    return "Entendi sua pergunta! Para informações específicas, recomendo navegar pelos módulos do sistema: Dashboard, Vendas, Estoque, Financeiro ou Relatórios. Cada seção tem dados detalhados sobre diferentes aspectos do negócio."
...
def chat():
    ...
    try:
        ...
        # Obter contexto atual do negócio
...
def health():
    """Endpoint de saúde do serviço de chat."""
    return jsonify({
        'status': 'healthy',

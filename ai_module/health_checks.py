...
Sistema avançado de health checks com:
- Validação detalhada de componentes
...
class HealthChecker:
    ...
    def check_database_health(self) -> ComponentHealth:
        ...
        try:
            # Simula verificação de database
            ...
            try:
                # Configurações simuladas (em produção usar .env)
                config = {
                    'host': 'localhost',
                    'database': 'padaria_db',
                    'user': 'root',
                ...
                if connection.is_connected():
                    ...
                    raise Error("Não foi possível conectar ao database")
    ...
    def check_ml_models_health(self) -> ComponentHealth:
        ...
        try:
            ...
            # Calcula tamanho médio dos modelos
            total_size = sum(
    ...
    def check_external_apis_health(self) -> ComponentHealth:
        ...
        for api_config in apis_to_test:
            ...
            try:
                ...
                metrics.append(HealthMetric(
                    ...
                    value=0,
                    unit="boolean",
                    status=HealthStatus.WARNING,  # APIs externas não são críticas
        ...
        if healthy_apis == total_apis:
            ...
            overall_status = HealthStatus.WARNING  # APIs são opcionais
    ...
    def check_system_health(self) -> ComponentHealth:
        """Verifica saúde do sistema operacional."""
        start_time = time.time()
        metrics = []
        details = {}
    ...
    def _get_metric_status(self, value: float, warning_threshold: float, 
        ...
        """Determina status de uma métrica baseado nos thresholds."""
        if inverted:
            # Para métricas onde valores menores são piores (ex: hit rate)
            ...
            # Para métricas onde valores maiores são piores (ex: latência)
            if value >= critical_threshold:
                return HealthStatus.CRITICAL
            elif value >= warning_threshold:
                return HealthStatus.WARNING
    ...
    def _get_system_overview_metrics(self) -> List[HealthMetric]:
        ...
        try:
            ...
            # Network connections (se disponível)
            try:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        """Retorna resumo rápido de saúde."""
        ...
        # Executa check rápido se necessário
...
def get_health_checker(logger=None):
    """Retorna instância do health checker."""
    if logger and not health_checker.logger:
        health_checker.logger = logger
    return health_checker

# Função utilitária para health check rápido
...
if __name__ == "__main__":
    ...
    print(f"🔧 Componentes: {health.healthy_components}/{health.total_components} saudáveis")
    
    print("\n📋 Componentes:")
    for component in health.components:
        # Determina ícone de status baseado no status do componente

...
def train_prophet_models(data_path, models_dir):
    """Treina um modelo Prophet para cada produto com otimização de hiperparâmetros."""
    ...
    if df.empty:
        print("DataFrame vazio. Não é possível treinar modelos.")
        return
    ...
    for product_name in unique_products:
        ...
        if not best_params:
            print(f"Falha na otimização para {product_name}. Usando parâmetros padrão.")
            best_params = {
        ...
        if metrics:
            print(f"Métricas de avaliação para {product_name}:")
        ...
        # Salva os parâmetros e métricas
        results = {
